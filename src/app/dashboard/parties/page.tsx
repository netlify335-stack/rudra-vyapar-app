...
export default async function PartiesPage() {
  ...
  const all = await db.select().from(parties).where(eq(parties.storeId, storeId)).orderBy(asc(parties.name));
...
function PartiesList({ title, parties }: { title: string; parties: Party[] }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="divide-y divide-slate-100">
        ...
        {parties.length === 0 && <div className="p-8 text-center text-sm text-slate-400">No {title.toLowerCase()} yet</div>}
      </div>
    </div>
  );
}
