...
export function PurchasesClient({ storeId }: { storeId: string }) {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const cList = await db.select({ id: parties.id, name: parties.name }).from(parties).where(and(eq(parties.storeId, storeId), eq(parties.type, "supplier")));

        setPurchases(dataList);
        setSuppliersList(cList);
      } catch (err) {
    ...
    load();
  }, [storeId, qStr, dateFilter, start, end, supplier, mode]);
