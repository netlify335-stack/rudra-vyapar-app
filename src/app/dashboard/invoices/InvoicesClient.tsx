...
export function InvoicesClient({ storeId }: { storeId: string }) {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const cList = await db.select({ id: parties.id, name: parties.name }).from(parties).where(and(eq(parties.storeId, storeId), eq(parties.type, "customer")));
    ...
    load();
  }, [storeId, qStr, dateFilter, start, end, customer, mode]);
