...
export function InvoiceDetailClient({ invoiceId, storeId }: { invoiceId: string, storeId: string }) {
  const [data, setData] = useState<{ inv: any, items: any[], store: any } | null>(null);
  ...
  if (!data) return <div className="p-8 text-center text-slate-500">Loading Invoice Details...</div>;

  const { inv, items, store } = data;
