...
export default function KhataDetailClient({ storeId, id, searchParams }: { storeId: string, id: string, searchParams: any }) {
  ...
  const { data, loading, error } = useLocalDbQuery(async (db) => {
    ...
    return { party, entries, store };
  }, [storeId, id, dateFilter, start, end, type]);

  if (loading) return <div>Loading...</div>;
  if (error) return <div>Error loading khata detail</div>;
  if (!data?.party) return <div>Party not found</div>;

  const { party, entries, store } = data;
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="text-right">
            ...
            {bal > 0 && party.phone && store && <RemindButton partyId={party.id} amount={bal} name={party.name} phone={party.phone} storeName={store.name} />}
          </div>
        </div>
      </div>

      <KhataAdd partyId={party.id} storeId={storeId} />
