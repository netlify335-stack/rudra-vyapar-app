...
export default function KhataClient({ storeId }: { storeId: string }) {
  const { data, loading, error } = useLocalDbQuery(async (db) => {
    ...
    return { customers, suppliers, overdueCount: overdueCount[0]?.count ?? 0 };
  }, [storeId]);

  if (loading) return <div>Loading...</div>;
  if (error || !data) return <div>Error loading khata</div>;

  const { customers, suppliers, overdueCount } = data;
      ...
      <div className="rounded-2xl border border-orange-200 dark:border-orange-900/30 bg-gradient-to-r from-orange-50 via-rose-50 to-amber-50 dark:from-orange-900/10 dark:via-rose-900/10 dark:to-amber-900/10 p-6 shadow-sm">
        <div className="grid gap-4 md:grid-cols-3">
          <div>
            ...
            <div className="text-xs text-slate-600 dark:text-slate-400">{overdueCount} parties pending</div>
          </div>
          <div>
