...
export function HistoryClient({
  ...
  storeId: string;
  searchParams: { filter?: string; start?: string; end?: string };
}) {
  ...
  const { data, loading, error } = useLocalDbQuery(
    async (db) => {
      ...
      return { totalSalesRow, netSalesRow, breakdown };
    },
    [storeId, startDate, endDate]
  );

  if (loading) return <div>Loading history...</div>;
  if (error) return <div className="text-red-500">Error loading history</div>;

  const totalSalesRow = data?.totalSalesRow || [];
  const netSalesRow = data?.netSalesRow || [];
